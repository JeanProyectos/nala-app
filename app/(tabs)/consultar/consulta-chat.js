...
export default function ConsultaChatScreen() {
  ...
  const loadConsultation = async () => {
    try {
      ...
      if (data.status === 'EXPIRED') {
        Alert.alert(
          'Consulta Expirada',
          'Esta consulta ha expirado. Por favor crea una nueva.',
          [
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      {(consultation.status === 'PENDING_PAYMENT' || consultation.status === 'IN_PROGRESS') && (
          ...
          <TouchableOpacity
            ...
            onPress={async () => {
              try {
                ...
                Alert.alert(
                  'Consulta Finalizada',
                  'La consulta ha finalizado. Ahora puedes proceder con el pago.',
                  [
                    { 
                      text: 'Pagar Ahora', 
                      onPress: () => router.push(`pago-consulta?id=${consultationId}`) 
                    },
